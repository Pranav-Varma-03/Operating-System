...
void entry_sec(){
    ...
    int new_val = 1;
        ...
        while (cas_lock.compare_exchange_strong( exp_val, new_val ) != true)
        {
            // Doing Nothing
            exp_val = 0;
            new_val = 1;
            // cout<<"exp value: "<<exp_val<<endl;
            // cout<<"lock value: "<<cas_lock<<endl;
        }
    return;
}
...
void* testCS(void *Data){
    ...
    t_data->waiting_time.push_back((double)(time_diff.count()));
        ...
        usleep(t1*1000);
        exit_sec(); //SLEEP Critical Section.
